...
\begin{table*}[h]
  ...
  \caption{Prediction results individual models. The baseline row states averaged results of our baseline classifier, the general row states the averaged results of model trained on all participants. Distribution presents the distribution of the binarized outcome variable.}\label{tab:accuracy}%\todo{revise caption}}
%  The distribution columns show a bar chart of the response distribution (negative/positive) for each of the three variables. The baseline row represents the averaged results of our baesline classifier. The general row shows the averaged results of our models trained on all participants.
  \vspace*{-4mm}
\end{table*}
...
\begin{table}[t!]
    ...
    \begin{tabular}{llll}
      ...
      \hline
    \end{tabular}
    \caption{Hyperparameters selected by grid search analysis for tuning our random forest models. K refers to the number of features selected.}\label{tab:hyperparams}
    \vspace*{-1cm}
    \end{centering}
\end{table}
...
\begin{table}[t!]
  ...
  \begin{tabular}{llll}
    ...
    \hline
  \end{tabular}
  \caption{Averaged Gini importance of each feature category per response variable.}
