...
After the initial setup, participants were asked to fill out three surveys each day for the following eight weeks (see Section~\ref{sec:Surveys}). At the end of the eight weeks, we collected the biometric sensors and performed a short follow-up interview on the participants' experiences.
...
\begin{figure}
  ...
  \caption{Biovotion's Everion}
   \label{everion}
\end{figure}

Table~\ref{signals} lists the biometric measurements that we collected using the Everion. Each measurement is collected once per second, and each recorded observation has an associated timestamp and quality rating. Data collected by the Everion is uploaded to a server, from which we downloaded the data for our study.
...
%\hline
\end{tabular}
\caption{Biometric measurements captured by the Everion}
\label{signals}
\end{center}
%\vspace*{-4mm}
\end{table}
