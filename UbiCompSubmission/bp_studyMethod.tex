...
After the initial setup, participants were asked to fill out 3 surveys each day for the following eight weeks (see Section~\ref{sec:Surveys}). At the end of the eight weeks, we collected the biometric sensors and performed a short follow-up interview on the study and the participants' experiences.
...
stress. By recording attendence of participants at the Tai Chi
sessions, we are able to analyze the impact of the sessions
on the results. As a brief summary, the impact of the
sessions was minimal; a a result, we present an analysis
of the effects in the Discussion section of the paper.
...
To gain a better understanding of our participants day-to-day work activities, we installed an open source computer interaction monitor~\footnote{reference omitted for double-blind review} \todo{One of the conditions to submit to Ubicomp is to NOT omit references due to anonymization purposes. So either we add the reference (which will be fine as long as we don't say we made it, or completely remove this reference)}. The monitor ran in the background on participants' computers and tracked the active windows, as well as the keyboard and mouse activity. Four of our participants opted out of this part of the study for privacy reasons (S6, S10, S12, and S13). %Therefore, we only included the computer interaction data in our analysis for our more coarse-grained explanatory model but not in our analysis on the predictive model for stress in the moment. 
